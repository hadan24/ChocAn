...
void service_list::read_from_disk(ifstream &ifile)
{
	service temp;
	temp.read_from_disk(ifile);
	list.push_back(temp);
}
...
void service::read_from_disk(ifstream &ifile)
{
	if (!ifile) {
		cerr << "\n\nError getting services from provider list.\n\n";
		ifile.close();
		return;
	}

	getline(ifile, date_of_service, '|');
	ifile >> code;	ifile.ignore(1, '|');
	getline(ifile, name, '|');
	getline(ifile, comments, '|');
	ifile >> cost;	ifile.ignore(1, '|');
	getline(ifile, current_date, '|');
}
